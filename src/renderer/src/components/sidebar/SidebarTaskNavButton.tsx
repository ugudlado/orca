...
import { EyeOff, Github, Gitlab, List, ListTodo } from 'lucide-react'
      ...
      <ContextMenuTrigger asChild>
        ...
        >
          ...
          <span className="hidden items-center gap-1 group-hover:flex group-focus-within:flex">
            ...
            {visibleTaskProviders.includes('backlog') ? (
              <TaskProviderShortcut
                canBrowseTasks={canBrowseTasks}
                label={translate(
                  'auto.components.sidebar.SidebarNav.open_backlog_tasks',
                  'Open Backlog tasks'
                )}
                onOpen={() => openTaskPage({ taskSource: 'backlog' })}
              >
