...
export default function SmartWorkspaceNameField({
  ...
  const showJiraSiteContext =
    (mode === 'smart' || mode === 'jira') && jiraConnectionStatus?.selectedSiteId === 'all'
  ...
  const jiraSearchJql =
    (mode === 'smart' || mode === 'jira') && !jiraSource.intent && sourceQueryWithinLimit
  ...
  const rows = useMemo<RowEntry[]>(() => {
    ...
    return buildSmartWorkspaceSourceRows({
      ...
      backlogAvailable,
      backlogTasks: getVisibleHeldProviderResults({
        items: backlogTasks,
        value,
        debouncedQuery
      }),
