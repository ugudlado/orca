...
function renderField(
  ...
  return render(
    <SmartWorkspaceNameField
      ...
      repoBackedSourcesDisabled={overrides.repoBackedSourcesDisabled}
    />
  )
}
...
describe('SmartWorkspaceNameField Jira accessibility', () => {
  ...
  it('shows Jira as a source mode only when the selected workspace host is connected', () => {
    ...
    // Why: per-provider tabs were removed — Jira results surface under Smart mode.
    expect(screen.getByRole('textbox').getAttribute('placeholder')).toContain('Jira')
  })
  ...
  it('labels all-site Jira results with the bound site and account', async () => {
    ...
    // Why: per-provider tabs were removed — Jira results now surface under Smart
    // mode, where GitHub link parsing also runs. Disable repo-backed sources so
    // "ORCA-123" isn't raced into a cross-repo GitHub prompt that closes the popover.
    renderField({ jiraSourceContext: true, value: 'ORCA-123', repoBackedSourcesDisabled: true })
